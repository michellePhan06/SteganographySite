...
# Minimum bits to skip at the start of each format to avoid corrupting file headers.
# These values skip past the mandatory header/magic bytes that image decoders
# validate strictly. Modifying even one bit in these regions corrupts the file.
HEADER_SKIP_BITS = {
    '.jpg':  4096,   # JPG: skip SOI + APP0/APP1 (EXIF) markers (~512 bytes to be safe)
    '.jpeg': 4096,
    '.png':  4096,   # PNG: skip 8-byte magic + IHDR chunk + first few chunks
    '.gif':  400,    # GIF: skip 6-byte header + logical screen descriptor
    '.bmp':  440,    # BMP: skip 14-byte file header + 40-byte DIB header
    '.webp': 480,    # WebP: skip RIFF header + VP8 chunk header
}
DEFAULT_HEADER_SKIP = 1024  # safe default for unknown formats (128 bytes)

def safe_start_bit(ext, user_start):
    """Return the larger of user_start and the format's required header skip."""
    minimum = HEADER_SKIP_BITS.get(ext.lower(), DEFAULT_HEADER_SKIP)
    return max(user_start, minimum)
...
# Steganography
# Generate sequence of bit positions where data will be embedded and extracted
...
def embed(carrier, message, start, periods, mode):
    ...
    # Check if carrier is large enough to store message
    if positions and positions[-1] >= len(carrier) * 8:
        raise ValueError(
            f"Message too large for carrier with these parameters. "
            f"Carrier is {len(carrier) * 8} bits, but embedding requires reaching bit {positions[-1]}."
        )
...
def submit():
    if request.method == 'POST':
        ...
        # Reject multiple periods when mode is fixed
        if mode == 'fixed' and len(periods) > 1:
            flash('Multiple periods are only allowed in Cycling mode. Switch Mode to Cycling or enter a single period value.')
            return redirect(url_for('submit'))
        ...
        # Determine file extension and enforce minimum header skip to prevent corruption
        ext = os.path.splitext(secure_filename(carrier.filename))[1].lower()
        actual_start = safe_start_bit(ext, start)
        if actual_start != start:
            flash(
                f'Start bit was automatically adjusted from {start} to {actual_start} '
                f'to protect the {ext} file header from corruption.'
            )

        # Attempt to embed message into carrier
        try:
            result = embed(carrier_bytes, message_bytes, actual_start, periods, mode)
        ...
        # Capture message file extension so we know how to serve it after extraction
        msg_ext = os.path.splitext(secure_filename(message.filename))[1].lower()

        # DB record — store the actual_start that was used, not the user's raw input
        post = Post(
            title=title,
...
def do_extract(post_id):
    ...
    post = Post.query.get_or_404(post_id)
    params = json.loads(post.params)
    msg_ext = params.get('msg_ext', '')
    is_msg_image = msg_ext in IMAGE_EXTS

    # Load stored carrier file
    ...
    try:
        ...
        if is_msg_image:
            # Encode extracted image as base64 so browser can display it inline
            b64 = base64.b64encode(msg).decode('ascii')
            mime = {
                '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg',
                '.png': 'image/png',  '.gif': 'image/gif',
                '.webp': 'image/webp', '.bmp': 'image/bmp',
            }.get(msg_ext, 'application/octet-stream')
            return jsonify(ok=True, type='image', mime=mime, b64=b64, ext=msg_ext)

        # Try to decode as UTF-8 text
        try:
            return jsonify(ok=True, type='text', content=msg.decode('utf-8'))
        # Fall back to hex for other binary formats
        except Exception:
            return jsonify(ok=True, type='binary', content=msg.hex(), ext=msg_ext)
